...
NeuroscopeApp::NeuroscopeApp()
    : QMainWindow(0), prefDialog(0L), displayCount(0), mainDock(0), spikeChannelPalette(0), tabsParent(0L), paletteTabsParent(0L),
      isInit(true), groupsModified(false), colorModified(false), eventsModified(false), initialOffsetDefault(0), propertiesDialog(0L), select(false), initialTimeWindow(0), undoRedoInprocess(false), isPositionFileLoaded(false)
{
...
void NeuroscopeApp::applyPreferences()
{
    ...
    if (traceBackgroundImageDefault != configuration().getTraceBackgroundImage())
    {
        ...
        if (!mainDock)
            doc->setTraceBackgroundImage(traceBackgroundImageDefault);
    }
    ...
    if (initialOffsetDefault != configuration().getOffset())
    {
        ...
        if (!mainDock)
            doc->setInitialOffset(initialOffsetDefault);
    }
    ...
    if (channelNbDefault != configuration().getNbChannels())
    {
        ...
        if (!mainDock)
            doc->setChannelNb(channelNbDefault);
    }
    ...
    if (resolutionDefault != configuration().getResolution())
    {
        ...
        if (!mainDock)
            doc->setResolution(resolutionDefault);
    }
    ...
    if (voltageRangeDefault != configuration().getVoltageRangeDefault())
    {
        ...
        if (!mainDock)
            doc->setGains(voltageRangeDefault, amplificationDefault, screenGainDefault);
    }
                    ...
                    if (eventsModified)
                    {
                        ...
                        if (!doc->saveEventFiles())
                        {
                            switch (QMessageBox::warning(0, tr("I/O Error !"), tr("The event file(s) could not be saved possibly because of insufficient file access permissions."
                                                                                  "Close anyway ?"),
                                                         QMessageBox::Close | QMessageBox::Discard))
                            {
...
void NeuroscopeApp::slotFileProperties()
{
    ...
    if (doc->isCurrentFileAdatFile())
        propertiesDialog->setEnabledCurrentSamplingRate(false);
    else
        propertiesDialog->setEnabledCurrentSamplingRate(true);
    ...
    { // execute the dialog
        ...
        {
            if (channelNb != propertiesDialog->getNbChannels())
                doc->setChannelNb(propertiesDialog->getNbChannels());
            if (SR != propertiesDialog->getSamplingRate())
                doc->setSamplingRate(propertiesDialog->getSamplingRate());
            if (acquisitionSystemSamplingRate != propertiesDialog->getAcquisitionSystemSamplingRate())
            {
                if (doc->isCurrentFileAdatFile())
                    doc->setSamplingRate(propertiesDialog->getAcquisitionSystemSamplingRate());
                else
                    doc->setAcquisitionSystemSamplingRate(propertiesDialog->getAcquisitionSystemSamplingRate());
            }
            if (resolution != propertiesDialog->getResolution())
                doc->setResolution(propertiesDialog->getResolution());
            if (offset != propertiesDialog->getOffset())
                doc->setInitialOffset(propertiesDialog->getOffset());
...
void NeuroscopeApp::slotDeselectAll()
{
    ...
    if (qobject_cast<ChannelPalette*>(current))
    {
        ...
        NeuroscopeView* view = activeView();
        if (editMode->isChecked())
            doc->deselectAllChannels(*view, true);
        else
            doc->deselectAllChannels(*view, false);
    }
    else
    {
        //Update the selected items of the current palette
...
void NeuroscopeApp::slotKeepChannels()
{
    ...
    NeuroscopeView* view = activeView();
    if (view->isSelectionTool())
        view->selectChannels(selectedChannels);
    else
        view->setSelectedChannels(selectedChannels);
            ...
            if (groupsModified || colorModified || eventsModified)
            {
                ...
                case QMessageBox::Save: //<=> Save
                    ...
                    if (eventsModified)
                    {
                        ...
                        if (!eventSaveStatus)
                        {
                            switch (QMessageBox::question(0, tr("I/O Error !"), tr("The event file(s) could not be saved possibly because of insufficient file access permissions."
                                                                                   "Close anyway ?"),
                                                          QMessageBox::Close | QMessageBox::Discard))
                            {
                            case QMessageBox::Close:
...
{
    if (QMessageBox::warning(this, tr("Synchronize anatomic and spike groups"), tr("This will overwrite all your spike groups by your anatomical groups. Do you wish to continue?"), QMessageBox::Ok | QMessageBox::Cancel) == QMessageBox::Cancel)
        return;
...
void NeuroscopeApp::createClusterPalette(const QString& clusterFileId)
{
    ...
    if (filePath.contains(".dat") || filePath.contains(".fil") || filePath.contains(".ns"))
    {
