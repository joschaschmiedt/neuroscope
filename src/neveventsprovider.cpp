...
NEVEventsProvider::NEVEventsProvider(const QString& fileUrl, int position)
    : EventsProvider(".nev.evt", 0, position), mExtensionHeaders(NULL)
{
...
int NEVEventsProvider::loadData()
{
    ...
    while (eventIndex + eventsSkipped < this->nbEvents)
    {
        ...
        case NEVDigitalSerialDataID:
            ...
            // Skip the padding
            if (!eventFile.seek(eventFile.pos() + mBasicHeader.data_package_size - sizeof(NEVDigitalSerialData) - sizeof(NEVDataHeader)))
                goto fail;
            break;
        case NEVConfigurationDataID:
            ...
            {
            case 0:
                label = EventDescription("config change normal");
                break;
            case 1:
                label = EventDescription("config change critical");
                break;
            default:
                label = EventDescription("config change undefined");
                break;
            }

            // Skip the config change data
            if (!eventFile.seek(eventFile.pos() + mBasicHeader.data_package_size - sizeof(NEVConfigurationDataHeader) - sizeof(NEVDataHeader)))
                goto fail;
            break;
        case NEVButtonDataID:
            ...
            {
            case 1:
                label = EventDescription("button press");
                break;
            case 2:
                label = EventDescription("button reset");
                break;
            default:
                label = EventDescription("button undefined");
                break;
            }

            // Skip the padding
            if (!eventFile.seek(eventFile.pos() + mBasicHeader.data_package_size - sizeof(NEVButtonData) - sizeof(NEVDataHeader)))
                goto fail;
            break;
        case NEVTrackingDataID:
            ...
            if (!eventFile.seek(eventFile.pos() + mBasicHeader.data_package_size - sizeof(NEVTrackingDataHeader) - sizeof(NEVDataHeader)))
                goto fail;
            break;
        case NEVVideoSyncDataID:
            ...
            // Skip the padding
            if (!eventFile.seek(eventFile.pos() + mBasicHeader.data_package_size - sizeof(NEVVideoSyncData) - sizeof(NEVDataHeader)))
                goto fail;
            break;
        case NEVCommentDataID:
            label = EventDescription("comment");

            if (!eventFile.seek(eventFile.pos() + mBasicHeader.data_package_size - sizeof(NEVDataHeader)))
                goto fail;
            break;
        default:
