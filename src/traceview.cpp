    ...
    else if (!eventProvidersToUpdate.isEmpty())
    {
        ...
        for (providerIterator = eventProvidersToUpdate.begin(); providerIterator != eventProvidersToUpdate.end(); ++providerIterator)
        {
            ...
            if (eventData == 0)
            {
                ...
                eventsData[*providerIterator]->setStatus(false);
        }
        QHashIterator<QString, EventData*> iterator(eventsData);
        while (iterator.hasNext())
        {
...
void TraceView::updateWindow()
{
    ...
    if (!multiColumns)
    {
        ...
        if (raster && nbClusters != 0)
        {
            ...
            ordinateMin = -(borderY + borderY / 2 + (nbGps - 1) * YGroupSpace + nbShownchannels * traceVspace + nbYspaces * Yspace + rasterTotalHeight);

            Y0 = borderY / 2 + (nbGps - 1) * YGroupSpace + nbShownchannels * traceVspace + nbYspaces * Yspace - (traceVspace / 2) + rasterTotalHeight;
    ...
    else
    {
        ...
        if (raster && nbClusters != 0)
        {
            ...
            ordinateMin = -(borderY + borderY / 2 + maxNbChannels * traceVspace + (maxNbChannels - 1) * Yspace + rasterTotalHeight);
            Y0 = borderY / 2 + maxNbChannels * traceVspace + (maxNbChannels - 1) * Yspace - (traceVspace / 2) + rasterTotalHeight;
    ...
    else
    {
        ...
        for (int i = 1; i < nbSpikes + 1; ++i)
        {
            ...
            if (clusterList.contains(clusterId))
            {
                ...
                {
                    if (firstIndex > traceInfo(1, j))
                        continue; //case 1
                    else if (firstIndex <= traceInfo(1, j))
                    { //case 2
                        for (int k = j; k <= nbSamplesToDraw; ++k)
                        {
                            ...
                            {
                                if (traceInfo(3, k) != traceInfo(4, k))
                                    painter.drawLine(traceInfo(2, k), traceInfo(3, k), traceInfo(2, k), traceInfo(4, k));
                                else
                                    painter.drawPoint(traceInfo(2, k), traceInfo(3, k));
                            }
                        }
                        break;
                    }
        ...
        for (iterator = groupIds.begin(); iterator != groupIds.end(); ++iterator)
        {
            ...
            if (verticalLines && nbClusters != 0)
            {
                ...
                for (selectedIterator = selectedClusters.begin(); selectedIterator != selectedClusters.end(); ++selectedIterator)
                {
                    ...
                    if (!clusterFileList.contains(selectedIterator.key()))
                        continue;
            ...
            for (int j = 0; j < currentNbChannels; ++j)
            {
                ...
                {
                    for (int i = 1; i <= nbSamples; ++i)
                        m += static_cast<long>(data(i, channelId + 1) * channelFactors.at(channelId));
                    m /= nbSamples;
                }
...
void TraceView::drawTimeLine(QPainter* painter)
{
    ...
    if (currentAbscissae[groupIndex] < min[groupIndex])
    {
        if (!initialDragLine)
        {
            ...
            if (!linePositions.isEmpty())
                for (int i = 0; i < nbColumns; ++i)
                    previousAbscissae.append(linePositions.at(i) + previousDelta);
    ...
    else
    {
        ...
        else
        {
            ...
            if (multiColumns)
            {
                //left margin is visible
                if (r.left() == 0)
                    relativeTime = static_cast<float>(fmod(x, static_cast<float>(Xshift))) / static_cast<float>(Xstep) * timeStep;
                //left margin is invisible
                else
                {
                    int nbSamples = tracesProvider.getNbSamples(startTime, endTime, startTimeInRecordingUnits);
                    ...
                    {
                        relativeTime = static_cast<float>(fmod((x), static_cast<float>(shift + XGroupSpace))) / static_cast<float>(Xstep) * timeStep;
                    }
                }
            }
            //single column
            ...
            else
            {
                ...
                //else time =  relativeTime + startTime;
                    ...
                    time = relativeTime + static_cast<float>(startTime);
        ...
        { //If the user went to far on the right, draw a line at the last sample position (max)
            if (!startEventDragging)
            {
                ...
                if (previousAbscissa < max)
                {
                    ...
                    {
                        previousDragAbscissa = x;
                        if (selectedEventPosition[1] + (previousDragAbscissa - lastClickAbscissa) > max)
                ...
                else if (r.left() == 0)
                {
                    ...
                    else
                    {
                        ...
                        if (samplePart <= 0)
                        {
                            ...
                            sampleIndex = static_cast<int>(floor(0.5 + static_cast<float>(samplePart) / static_cast<float>(Xstep)) * downSampling);
                    }
                }
                //left margin is invisible
                else
    ...
    else if (mode == SELECT_TIME && (event->button() & Qt::LeftButton))
    {
        ...
        else
        {
            if (multiColumns)
            {
                ...
                if (r.left() == 0)
                {
                    relativeStartingTime = static_cast<float>(fmod(startingIndex, static_cast<float>(Xshift))) / static_cast<float>(Xstep) * timeStep;
                    if (startingIndex < Xshift)
                        columnNb = 1;
                    else
                        columnNb = ((startingIndex - (Xshift)) / Xshift) + 2;
                }
                //left margin is invisible
                else
                {
                    ...
                    {
                        relativeStartingTime = static_cast<float>(fmod((startingIndex), static_cast<float>(shift + XGroupSpace))) / static_cast<float>(Xstep) * timeStep;
            ...
            //between columns
            if (relativeStartingTime > timeFrameWidth)
                relativeStartingTime = 0;
        }
        ...
        else
        {
            if (multiColumns)
            {
                ...
                if (r.left() == 0)
                {
                    relativeEndingTime = static_cast<float>(fmod(x, static_cast<float>(Xshift))) / static_cast<float>(Xstep) * timeStep;
                    int currentColumnNb;
                    if (x < Xshift)
                        currentColumnNb = 1;
                    else
                        currentColumnNb = ((x - (Xshift)) / Xshift) + 2;
                    ...
                    if (currentColumnNb > columnNb && relativeStartingTime != 0)
                        relativeEndingTime = timeFrameWidth;
                    if (currentColumnNb < columnNb)
                ...
                else
                {
                    ...
                    if (x < shift)
                    {
                        ...
                        if (columnNb == 1 && relativeEndingTime < relativeStartingTime)
                        { //the user went backwards on the first column
                            startingTime = startTime + static_cast<int>(0.5 + relativeEndingTime);
                        }
                    }
                    else
                    {
                        relativeEndingTime = static_cast<float>(fmod((x), static_cast<float>(shift + XGroupSpace))) / static_cast<float>(Xstep) * timeStep;
            ...
            //between columns
            if (relativeEndingTime > timeFrameWidth)
                relativeEndingTime = timeFrameWidth;
        }
        ...
        if (duration > timeFrameWidth)
            duration = timeFrameWidth;
...
void TraceView::drawCalibrationScale(QPainter& painter)
{
    ...
    if (!multiColumns)
        ...
        width = static_cast<float>(worldToViewportWidth(Xshift) - worldToViewportWidth(XGroupSpace));
        ...
        else
        {
            ...
            if (multiColumns)
            {
                ...
                if (r.left() == 0)
                {
                    ...
                    if (windowWidth <= traceWidth)
                        newWidth = static_cast<int>((windowWidth)*previousDownSampling / downSampling) * Xstep;
                    else
                    {
                        ...
                        {
                            newWidth = static_cast<int>((windowWidth - (nbGroupParts * previousXGroupSpace)) * previousDownSampling / downSampling) * Xstep + (nbGroupParts * XGroupSpace);
                        }
                        else
                        {
                            int marginPart = static_cast<int>(XGroupSpace * static_cast<float>(secondPart - traceWidth) / static_cast<float>(previousXGroupSpace));
                            newWidth = static_cast<int>((windowWidth - (nbGroupParts * previousXGroupSpace) - (secondPart - traceWidth)) * previousDownSampling / downSampling) * Xstep + (nbGroupParts * XGroupSpace) + marginPart;
                        }
                    }
                }
                else
                {
                    ...
                    if (r.left() <= Xshift)
                    {
                        ...
                        if (r.left() > traceWidth)
                        {
                            ...
                            if (withoutFirstMarginPart <= traceWidth)
                                newWidth = static_cast<int>(static_cast<float>(withoutFirstMarginPart) * previousDownSampling / downSampling) * Xstep + (XGroupSpace - marginPart);
                            else
                            {
                                ...
                                if (secondPart <= traceWidth)
                                {
                                    newWidth = static_cast<int>(static_cast<float>(withoutFirstMarginPart - (nbGroupParts * previousXGroupSpace)) * previousDownSampling / downSampling) * Xstep + (nbGroupParts * XGroupSpace) + (XGroupSpace - marginPart);
                                }
                                else
                                {
                                    ...
                                    newWidth = static_cast<int>(static_cast<float>(withoutFirstMarginPart - (nbGroupParts * previousXGroupSpace) - (secondPart - traceWidth)) * previousDownSampling / downSampling) * Xstep + (nbGroupParts * XGroupSpace) + (XGroupSpace - marginPart) + secondMarginPart;
                                }
                            }
                        }
                        //r.left() <= traceWidth
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if (secondPart <= previousXGroupSpace)
                                {
                                    ...
                                    newWidth = static_cast<int>((static_cast<float>(windowWidth - (nbGroupParts * previousXGroupSpace) - secondPart) * previousDownSampling / downSampling)) * Xstep + (nbGroupParts * XGroupSpace) + marginPart;
                                }
                                else
                                {
                                    newWidth = static_cast<int>((static_cast<float>(windowWidth - ((nbGroupParts + 1) * previousXGroupSpace)) * previousDownSampling / downSampling)) * Xstep + ((nbGroupParts + 1) * XGroupSpace);
                                }
                            }
                        }
                    } //r.left() <= Xshift
                    ...
                    else
                    {
                        ...
                        {
                            newLeft = static_cast<int>(static_cast<float>(r.left() - (groupIndex * previousXGroupSpace)) * previousDownSampling / downSampling) * Xstep + (groupIndex * XGroupSpace);
                            //Compute the width of the window
                        ...
                        else
                        {
                            ...
                            newLeft = static_cast<int>(static_cast<float>(r.left() - (groupIndex * previousXGroupSpace) - (samplePart - traceWidth)) * previousDownSampling / downSampling) * Xstep + (groupIndex * XGroupSpace) + marginPart;
                            //Compute the width of the window
                            ...
                            if (secondPart <= traceWidth)
                            {
                                newWidth = static_cast<int>(static_cast<float>(windowWidth - firstMargin - (nbGroupParts * previousXGroupSpace)) * previousDownSampling / downSampling) * Xstep + (nbGroupParts * XGroupSpace) + firstMarginPart;
                            }
                            else
                            {
                                int secondMarginPart = static_cast<int>(XGroupSpace * static_cast<float>(secondPart - traceWidth) / static_cast<float>(previousXGroupSpace));
                                newWidth = static_cast<int>(static_cast<float>(windowWidth - firstMargin - (secondPart - traceWidth) - (nbGroupParts * previousXGroupSpace)) * previousDownSampling / downSampling) * Xstep + (nbGroupParts * XGroupSpace) + firstMarginPart + secondMarginPart;
                            }
                        }
                    }
                }
...
void TraceView::print(QPainter& printPainter, int width, int height, bool whiteBackground)
{
    ...
    if (r.left() != 0)
        ...
        viewport = QRect(printPainter.viewport().left() + xMargin, printPainter.viewport().top(), printPainter.viewport().width() - xMargin, printPainter.viewport().height());
    ...
    //Draw channel ids and amplitude on the left side.
    if (showLabels)
        drawChannelIdsAndGain(printPainter);

    //Draw the calibration scale
    if (showCalibrationScale)
        drawCalibrationScale(printPainter);
        ...
        for (providersIterator = selectedEvents.begin(); providersIterator != selectedEvents.end(); ++providersIterator)
        {
            ...
            else
            {
                ...
                for (shownEventsIterator = selectedIds.begin(); shownEventsIterator != selectedIds.end(); ++shownEventsIterator)
                    if (!idsToNotUse.contains(*shownEventsIterator))
                        ids.append(*shownEventsIterator);
                if (ids.size() != 0)
                {
        ...
        for (providersIterator = selectedClusters.begin(); providersIterator != selectedClusters.end(); ++providersIterator)
        {
            ...
            else
            {
                ...
                for (shownClustersIterator = selectedIds.begin(); shownClustersIterator != selectedIds.end(); ++shownClustersIterator)
                    if (!idsToNotUse.contains(*shownClustersIterator))
                        ids.append(*shownClustersIterator);
                if (ids.size() != 0)
                {
                    idsToBrowse.insert(providersIterator.key(), ids);
